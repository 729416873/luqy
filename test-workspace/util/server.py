...
class Server():
    ...
    def create_start_cmd_list(self,i):
        ...
        cmd_list = []
        cmd = 'appium -p ' + str(self.pport[i]) + ' -bp ' + str(self.bpport[i]) + ' -U '+str(self.get_devices()[i])+' --no-reset --session-override'
        cmd_list.append(cmd)

        self.write_user_command.write_command(i,str(self.pport[i]),str(self.bpport[i]),str(self.get_devices()[i]))
        print(cmd_list)
        return cmd_list
    ...
    def kill_server(self):
        ...
        server_list = self.dos_cmd.execute_cmd_result('tasklist | find "node.exe"')
