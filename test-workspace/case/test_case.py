#coding=UTF-8
import sys
sys.path.append('D:/python/pythonlearning/test-workspace')
import unittest
...
class CaseTest(NewCaseTest):# 继承unittest.TestCase
    @classmethod
    def setUpClass(cls):
        global localbussiness
        localbussiness = LoginBusiness(Parame)
    ...
    def test_01(self):
        flag = localbussiness.login_username_error()
...
def get_suite(i):# (i,loc)进程加锁
    ...
    filename = 'D:/python/pythonlearning/test-workspace/report/test_report'+str(now)+' '+str(i)+'.html'
    fp = open(filename, 'wb')
